<template>
    ...
    <div class="slider-container" ref="sliderContainer">
        <div class="card" v-for="(logo, index) in logos" :key="index">
            <img :src="logo.image" alt="" class="logo">
        </div>
    </div>
    </div>
</template>
...
const props = defineProps(['direction', 'images', 'title']);

const logos = ref(props.images.map(image => ({ image })));

const rando = ref(randomFromInterval(.1, .2))
...
const setupHorizontalLoop = () => {
  const items = sliderContainer.value.children;
